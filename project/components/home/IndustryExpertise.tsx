...
const getDynamicIcon = (title: string): LucideIcon => {
  const name = title.toLowerCase();
  if (name.includes('health') || name.includes('medical') || name.includes('care') || name.includes('hospital')) return HeartPulse;
  if (name.includes('real estate') || name.includes('property') || name.includes('home')) return Home;
  if (name.includes('construction') || name.includes('architecture') || name.includes('engineering')) return HardHat;
  if (name.includes('ecommerce') || name.includes('e-commerce') || name.includes('retail') || name.includes('shop')) return ShoppingBag;
  if (name.includes('tech') || name.includes('software') || name.includes('it') || name.includes('saas')) return Cpu;
  if (name.includes('finance') || name.includes('bank') || name.includes('insurance') || name.includes('loan')) return Landmark;
  if (name.includes('education') || name.includes('learning') || name.includes('school') || name.includes('ed-tech')) return GraduationCap;
  if (name.includes('travel') || name.includes('tour') || name.includes('hotel')) return Plane;
  if (name.includes('hospitality')) return ConciergeBell;
  if (name.includes('food') || name.includes('restaurant') || name.includes('beverage') || name.includes('cafe')) return Utensils;
  if (name.includes('automotive') || name.includes('car') || name.includes('vehicle') || name.includes('transport')) return Car;
  if (name.includes('logistics') || name.includes('distribution') || name.includes('supply')) return Truck;
  if (name.includes('manufactur')) return Cog;
  if (name.includes('political') || name.includes('politics')) return Users;
  if (name.includes('consult')) return Handshake;
  if (name.includes('legal') || name.includes('law')) return Scale;
  if (name.includes('professional service')) return Briefcase;
  return Building2; // Agar kuch match na ho toh default
};
...
export default function IndustryExpertise() {
  ...
  useEffect(() => {
    const fetchIndustries = async () => {
      try {
        const response = await fetch(`${BASE_URL}/industries?limit=15&status=published`);
        const result = await response.json();
  ...
  return (
    <section className="bg-teal-50/50 py-20 lg:py-28">
      <div className="container mx-auto px-4">
        <SectionHeading
          eyebrow="Industry"
        ...
        {loading ? (
          ...
          <Stagger
            className="mt-16 grid grid-cols-2 divide-y divide-teal-100 sm:grid-cols-3 lg:grid-cols-5 lg:divide-x"
            stagger={0.05}
          >
            {industries.map((ind) => {
              const Icon = getDynamicIcon(ind.title);
              ...
              return (
                <StaggerItem key={ind._id}>
                  <Link
                    href={`/industries/${ind.slug}`}
                    className="group flex flex-col items-center gap-4 px-4 py-8 text-center transition-colors"
                  >
                    <div className="flex h-20 w-20 items-center justify-center rounded-full bg-teal-500 text-white shadow-sm transition-transform duration-300 group-hover:scale-105 group-hover:bg-teal-600">
