    ...
    <section className="bg-white py-20 lg:py-28">
      <div className="container mx-auto px-4">
        <SectionHeading
          ...
          description="Throughout our years of experience as a digital marketing agency, we've earned quite a few awards. Take a look at some of them here:"
        />

        <Stagger className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3" stagger={0.08}>
          {awards.map((award) => (
            <StaggerItem key={award.title}>
              <div className="flex h-full flex-col items-start gap-5 rounded-xl border border-slate-200 bg-white p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                {/* Award Badge/Logo */}
                <div className="relative h-24 w-24 flex-shrink-0">
                  {award.img ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center rounded-full bg-slate-50">
                      <AwardIcon className="h-10 w-10 text-slate-400" />
                    </div>
                  )}
                </div>

                <div>
                  {/* Organization + Star Rating */}
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-base font-bold text-electric">
                      {award.organization}
                    </span>
